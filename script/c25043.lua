--毒✿梅蒂欣·梅兰可莉
function c25043.initial_effect(c)
	--special summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(25043,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND)
	e1:SetCountLimit(1,25043)
	e1:SetCost(c25043.spcost)
	e1:SetTarget(c25043.sptg)
	e1:SetOperation(c25043.spop)
	c:RegisterEffect(e1)
	--search
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(25043,1))
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetCondition(c25043.thcon)
	e2:SetTarget(c25043.thtg)
	e2:SetOperation(c25043.thop)
	e2:SetLabelObject(e1)
	c:RegisterEffect(e2)
	--poison
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(25043,2))
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e3:SetCode(EVENT_BATTLE_START)
	e3:SetCountLimit(1,25043)
	e3:SetTarget(c25043.destg)
	e3:SetOperation(c25043.desop)
	c:RegisterEffect(e3)
	--Attribute
...
function c25043.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c25043.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c25043.thcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SPECIAL+1
end
function c25043.thfilter(c)
	return c:IsSetCard(0x164) and c:IsType(TYPE_TRAP) and c:IsSSetable()
end
function c25043.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c25043.thfilter,tp,LOCATION_DECK,0,1,nil) end
end
...
function c25043.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
end
function c25043.desop(e,tp,eg,ep,ev,re,r,rp)
	local e2=Effect.CreateEffect(e:GetHandler())
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e2:SetCountLimit(1)
	e2:SetCode(EVENT_PHASE+PHASE_STANDBY)
	e2:SetLabel(0)
	e2:SetCondition(c25043.damcon)
	e2:SetOperation(c25043.damop)
	e2:SetReset(RESET_PHASE+PHASE_STANDBY+RESET_OPPO_TURN,3)
	Duel.RegisterEffect(e2,tp)
end
function c25043.damcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()~=tp
end
function c25043.damop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local ct=e:GetLabel()
	if(ct<3) then
		ct=ct+1
