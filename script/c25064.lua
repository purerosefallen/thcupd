...
function c25064.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectMatchingCard(tp,c25064.rfilter1,tp,LOCATION_GRAVE,0,1,1,nil)
	if g:GetFirst():IsSetCard(0x740) then
		if g:GetFirst():IsAttribute(ATTRIBUTE_LIGHT)
			and Duel.IsExistingMatchingCard(c25064.rfilter3,tp,LOCATION_GRAVE,0,2,g:GetFirst()) and Duel.SelectYesNo(tp,aux.Stringid(25064,2)) then
			local g1=Duel.SelectMatchingCard(tp,c25064.rfilter3,tp,LOCATION_GRAVE,0,2,2,g:GetFirst())
			g:Merge(g1)
		...
		g1=Duel.SelectMatchingCard(tp,c25064.rfilter1,tp,LOCATION_GRAVE,0,2,2,g:GetFirst())
		g:Merge(g1)
		Duel.Remove(g,POS_FACEUP,REASON_COST)
	end
end
