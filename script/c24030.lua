...
function c24030.bantg(e,c)
	local code=e:GetHandler():GetFlagEffectLabel(24030)
	return code and c:IsCode(code) and (not c:IsOnField() or c:GetRealFieldID()>e:GetFieldID())
end
function c24030.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,564)
	local ac=Duel.AnnounceCard(tp)
	local code=e:GetHandler():GetFlagEffectLabel(24030)
	if e:GetHandler():GetFlagEffect(24030)>0 then
		e:GetHandler():SetFlagEffectLabel(24030,ac)
	else
		e:GetHandler():RegisterFlagEffect(24030,0x1fe1000,0,1,ac)
	end
